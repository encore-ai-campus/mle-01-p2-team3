...
트리플 추출 결과는 `data/quality` 폴더에 저장된 검증 산출물을 기준으로 평가했습니다. 전체 추출 트리플 26,072건 중 온톨로지 스키마 검사는 100% 통과했으며, 원천 행의 근거 문장과 일치하는 트리플은 25,692건으로 확인되었습니다.

| 지표 | 결과 | 해석 |
| --- | ---: | --- |
| 전체 트리플 수 | 26,072건 | 검증 대상 전체 관계 추출량 |
| 스키마 통과 트리플 수 | 26,072건 | 관계명과 source/target 타입이 온톨로지 규칙을 충족 |
| 스키마 준수율 | 100.00% | 정의한 그래프 스키마 기준으로 구조 오류 없음 |
| 근거 원문 일치 트리플 수 | 25,692건 | `evidence`가 원천 행에서 확인된 트리플 |
| 근거 원문 일치율 | 98.54% | 대부분의 관계가 원문 근거와 함께 추출됨 |
| 기각 트리플 수 | 380건 | 원천 식별자 또는 원천 행 매칭 실패로 제외 |
| 수동 검토 대상 수 | 25,692건 | 스키마와 근거 검증을 모두 통과한 최종 검토 후보 |

#### 관계 유형별 분포

| 관계 유형 | 건수 | 설명 |
| --- | ---: | --- |
| `IN_INDUSTRY` | 8,952건 | 기업과 산업 분류 연결 |
| `HAS_SUBSIDIARY` | 7,821건 | 모기업과 종속기업 연결 |
| `LOCATED_IN` | 7,346건 | 기업과 지역 정보 연결 |
| `AFFILIATED_WITH` | 1,573건 | 기업과 계열 관계 연결 |
| `RELATED_TO` | 380건 | 뉴스 기반 관련 관계 후보 |

#### 주요 오류 유형

기각된 380건은 모두 `missing_source_identifier`와 `source_row_not_found`에 해당했습니다. 즉 관계 구조 자체의 문제라기보다, 일부 트리플에서 원천 데이터 행을 추적할 수 있는 식별자가 누락되었거나 해당 원천 행을 다시 찾지 못한 것이 주요 원인이었습니다.

#### 검증 산출물

- `data/quality/quality_summary.json`: 품질 지표 요약
- `data/quality/quality_report.md`: 트리플 품질 검증 리포트
- `data/quality/approved_triples.jsonl`: 스키마와 근거 검증을 통과한 트리플
- `data/quality/rejected_triples.jsonl`: 검증에서 제외된 트리플
- `data/quality/manual_review_sample_50.csv`: 사람이 직접 정밀도를 확인하기 위한 50건 샘플

#### GDS 그래프 분석 결과

품질 검증을 통과한 그래프를 Neo4j GDS 기준으로 분석한 결과, `companyGraph`는 8,190개 노드와 51,650개 관계로 구성되었습니다. PageRank 기준 핵심 허브는 `서울`, `금융`, `제조`, `경기`, `서비스` 순으로 나타났으며, Leiden 커뮤니티 탐지에서는 총 77개 커뮤니티가 확인되었습니다. 가장 큰 커뮤니티는 2,640개 노드를 포함해 기업, 지역, 산업 노드가 함께 묶이는 구조를 보였습니다.

관련 산출물은 `data/quality/gds_report.md`, `data/quality/gds_report.json`, `data/quality/gds_node_properties.jsonl`에 저장되어 있습니다.
