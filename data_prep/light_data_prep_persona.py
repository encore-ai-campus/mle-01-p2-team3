...
5. king 관련 episode 분류에는 agents.name, speaker, speech mention뿐 아니라 agents.persona mention도 별도 컬럼으로 포함합니다.
"""
...
def agent_persona_texts(ep: Dict[str, Any]) -> List[str]:
    """episode의 agents 필드에서 persona 텍스트만 추출합니다.

    원본 구조 예:
        ep["agents"] = [
            {"name": "servant", "persona": "I serve the king."},
            {"name": "guard", "persona": "I protect the castle."},
        ]

    반환값:
        ["I serve the king.", "I protect the castle."]

    사용 목적:
    - agents.name, speaker, speech에는 king이 없어도 persona 설명 안에서
      king과 관련된 episode를 찾기 위해 사용합니다.
    - persona는 실제 대화가 아니라 character의 배경 설정이므로,
      speech mention과 별도 컬럼으로 분리해서 저장합니다.
    """
    personas: List[str] = []
...
def count_persona_mentions(
    ep: Dict[str, Any],
    mention_index: Dict[str, List[Tuple[str, ...]]],
) -> Counter:
    """episode의 agents.persona 전체에서 character mention 횟수를 계산합니다.

    이 함수는 speech가 아니라 persona만 대상으로 합니다.

    예:
        servant persona = "I serve the king."

    결과:
        Counter({"king": 1})

    주의:
    - persona mention은 character 배경 설명 기반의 관련성입니다.
    - 실제 episode dialogue에서 발생한 mention이 아니므로
      king_mention_count와 합치지 않고 king_persona_mention_count로 별도 저장합니다.
    """
    persona_text = "\n".join(agent_persona_texts(ep))
    return count_mentions(persona_text, mention_index)
...
def classify_king_episode(ep: Dict[str, Any], mention_index) -> Dict[str, Any]:
    ...
    strong 기준:
    - agents.name에 king이 직접 등장하거나
    - turn-level speaker가 king이거나
    - speech에서 king이 충분히 언급되거나
    - agents.persona에서 king 관련성이 확인되면 strong으로 분류합니다.

    중요 구분:
    - king_mention / king_mention_count:
      실제 대화(speech) 안에서 king이 언급된 정보입니다.
    - king_persona_mention / king_persona_mention_count:
      character 배경 설명(agents.persona) 안에서 king이 언급된 정보입니다.

    persona는 episode의 실제 dialogue가 아니므로 speech mention과 합치지 않고
    별도 컬럼으로 저장합니다. 이렇게 해야 후속 팀원이 검색/분석 단계에서
    persona 기반 관련성을 따로 필터링할 수 있습니다.
    """
    agents = set(agent_names(ep))
    speakers = speaker_names(ep)

    # 실제 대화 텍스트에서 character mention을 계산합니다.
    speech_mentions = count_mentions("\n".join(speech_turns(ep)), mention_index)

    # agents.persona 텍스트에서 character mention을 별도로 계산합니다.
    # persona는 배경 설정이므로 speech_mentions와 섞지 않습니다.
    persona_mentions = count_persona_mentions(ep, mention_index)

    # 1) 구조화 필드 기반 king 관련성: 가장 신뢰도가 높습니다.
    direct = FOCUS_CHARACTER in agents
    speaker = FOCUS_CHARACTER in speakers

    # 2) speech 기반 king mention: 실제 dialogue에서 나온 언급입니다.
    speech_mention_count = speech_mentions.get(FOCUS_CHARACTER, 0)
    speech_mention = speech_mention_count > 0

    # 3) persona 기반 king mention: character 배경 설명에서 나온 언급입니다.
    persona_mention_count = persona_mentions.get(FOCUS_CHARACTER, 0)
    persona_mention = persona_mention_count > 0

    # relevance score는 direct/speaker를 가장 강하게 보고,
    # speech mention과 persona mention은 보조 신호로 반영합니다.
    # persona mention은 배경 설명이므로 과도하게 커지지 않도록 최대 3점까지만 반영합니다.
    score = (
        int(direct) * 5
        + int(speaker) * 3
        + min(speech_mention_count, 5)
        + min(persona_mention_count, 3)
    )

    # 대표 relation_type은 가장 강한 근거를 우선합니다.
    if direct:
        relation_type = "direct_appearance"
    elif speaker:
        relation_type = "speaker"
    elif speech_mention:
        relation_type = "speech_mention"
    elif persona_mention:
        relation_type = "persona_mention"
    else:
        relation_type = "none"
    ...
    return {
        ...
        # 기존 speech mention 컬럼입니다.
        "king_mention": speech_mention,
        "king_mention_count": speech_mention_count,

        # persona mention 컬럼입니다. speech mention과 분리해서 저장합니다.
        "king_persona_mention": persona_mention,
        "king_persona_mention_count": persona_mention_count,

        # 최종 king 관련성 점수와 분류 결과입니다.
        "king_relevance_score": score,
        "is_king_related": direct or speaker or speech_mention or persona_mention,
        "is_king_strong": direct or speaker or score >= 2 or persona_mention,
...
def build_graphrag_doc(ep: Dict[str, Any], flags: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # persona는 실제 dialogue가 아니라 character 배경 설명입니다.
    # persona에서만 king 관련성이 발견된 episode를 후속 팀원이 해석할 수 있도록 문서에도 포함합니다.
    if personas:
        parts += [
            "",
            "[Personas]",
    ...
    parts += [
        ...
        f"Speech king mention count: {flags['king_mention_count']}",
        f"Persona king mention count: {flags['king_persona_mention_count']}",
        ...
        if flags["is_king_related"]:
            record = {
                ...
                **flags,
                ...
                # persona에서 king 관련성이 잡힌 episode를 검토할 수 있도록 원문 persona를 함께 저장합니다.
